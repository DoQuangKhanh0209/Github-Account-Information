...
final class AccountUserDetails: UIViewController {
    ...
    private let network = APIService.shareDataAPI
    ...
    private func initListRepo() {
        userRepository.getAllRepositoryOfUser(urlApi: userInfo?.reposUrl ?? "") { [weak self] (data, error) -> (Void) in
            guard let self = self else { return }
            if let _ = error {
                return
            }
    ...
    private func initAvatarImage() {
        network.getImageUser(imageURL: (userInfo?.avatarUrl) ?? "") { [weak self] (data, error)  in
            guard let self = self else { return }
            if let error = error {
                print(error)
                return
    ...
    private func setImageForFavoriteButton() {
        ...
        favoriteImageView.tintColor = isFavorite ? .red: .white
    }
...
extension AccountUserDetails: UITableViewDataSource {
